import InteractiveMathGame from "@/components/games/interactive-math";
import { Metadata } from "next";
import { getTranslations } from "next-intl/server";
import { LocaleLayoutProps } from "../../layout";
import { absoluteURL, getOgImage } from "@/lib/utils";
import { languages } from "@/i18n/config";

export const generateMetadata = async({params}: LocaleLayoutProps): Promise<Metadata> => {
     const {locale} = await params
     const t = await getTranslations("interactive-math");
     const gamesTxt = await getTranslations("games")
     return {
          title: t("title"),
          alternates: {
               languages: Object.fromEntries(languages.map(l => [l.code, `/${l.code}/games/interactive-math`])),
               canonical: absoluteURL(`/${locale}/games/interactive-math`),
          },
          openGraph: {
               title: t("title"),
               url: absoluteURL(`/${locale}/games/interactive-math`),
               locale,
               siteName: `ArsenKids ${gamesTxt("metaTitle")}`,
               type: "website",
               images: {
                    url: getOgImage(),
                    width: 1200,
                    height: 630
               }
          },
          twitter: {
               title: t("title"),
               card: "summary_large_image",
               images: [{
                    url: getOgImage(),
                    width: 1200,
                    height: 630
               }]
          }
     }
}

export default function InteractiveMath(){
     return (
          <InteractiveMathGame/>
     )
}